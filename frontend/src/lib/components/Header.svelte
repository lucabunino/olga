<script>
    ...
    let menuOpen = $state(false); // Using $state for Svelte 5
</script>

<header>
    <nav class="{page.url.pathname === '/' ? 'bg-gray' : undefined}" aria-label="Main navigation">
        <a href="/" class="logo" aria-label="Studio Olga home" onclick={() => {menuOpen = false}}>
        ...
        <button 
            class="menu-switch {menuOpen ? 'active' : ''}" 
        ...
        <ul 
            id="main-menu" 
            class="menu md-26-mb {menuOpen ? 'open' : 'closed'}"
            aria-hidden={innerWidth.current <= 768 && !menuOpen}
        >
            <li><a aria-current={page.url.pathname.startsWith('/studio') ? 'page' : undefined} href="/studio" onclick={() => {menuOpen = false}}>studio</a></li>
            <li><a aria-current={page.url.pathname.startsWith('/portfolio') ? 'page' : undefined} href="/portfolio" onclick={() => {menuOpen = false}}>portfolio</a></li>
            <li><a aria-current={page.url.pathname.startsWith('/contatti') ? 'page' : undefined} href="/contatti" onclick={() => {menuOpen = false}}>contatti</a></li>
        </ul>
    </nav>
</header>
